...
def get_dataset_snippet(request: HttpResponse, dataset_id):
    ...
    if dataset.dataset_type == DatasetTypes.CSV.value:
        return load_snippet_from_disk(dataset)
    elif dataset.dataset_type == DatasetTypes.HUGGING_FACE.value:
        return load_snippet_from_hugging_face(dataset)
    

def load_snippet_from_disk(dataset : Dataset):
...
def load_snippet_from_hugging_face(dataset : Dataset):
    ds = load_dataset(dataset.dataset_name, dataset.metadata['config'], split=dataset.metadata['split'])
    ds_sample = ds.shuffle().select(range(10))
    
    buffer = BytesIO()
    ds_sample.to_csv(buffer, header=True, index=False)
    buffer.seek(0)
